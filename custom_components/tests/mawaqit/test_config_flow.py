...
async def test_step_user_one_instance_allowed(hass: HomeAssistant):
    ...
    flow.hass = hass
    with patch('homeassistant.components.mawaqit.config_flow.is_data_folder_empty', return_value=False):
...
async def test_async_step_user_connection_error(hass: HomeAssistant):
    ...
    connection_error_instance = aiohttp.client_exceptions.ClientConnectorError(mock_conn_key, mock_os_error)
...
async def mock_mosques_test_data():
    ...
    mocked_mosques_data = (
        ['Mosque1-label (1.74km)', 'Mosque2-label (20.0km)', 'Mosque3-label (20.0km)'],  # name_servers
        [
...
async def test_async_step_mosques(hass, mock_mosques_test_data):
    ...
    with patch("homeassistant.components.mawaqit.config_flow.get_mawaqit_token_from_file",return_value="TOKEN",), \
        ...
        mosque_uuid_label = mocked_mosques_data[0][0]  # Assuming the user selects the first mosque
...
async def config_entry_setup(hass: HomeAssistant):
    ...
    entry = config_entries.ConfigEntry(
        ...
        data={'api_key': 'TOKEN', 'uuid': 'aaaaa-bbbbb-cccccc-0000', 'latitude': 32.87336, 'longitude': -117.22743},
...
@pytest.mark.asyncio
async def test_options_flow_valid_input(hass: HomeAssistant, config_entry_setup, mock_mosques_test_data):
    ...
    with patch('homeassistant.components.mawaqit.config_flow.read_all_mosques_NN_file', return_value=mocked_mosques_data), \
        ...
        mosque_uuid_label = mocked_mosques_data[0][1]  # Assuming the user selects the first mosque
        mosque_uuid = mocked_mosques_data[1][1]  # uuid of the first mosque

        #TODO chage this if we remove the create_entry line 278
        result = await flow.async_step_init(user_input={CONF_CALC_METHOD: mosque_uuid_label })
        assert result["type"] == data_entry_flow.RESULT_TYPE_CREATE_ENTRY  # Assert that an entry is created/updated
        # assert result["data"][CONF_UUID] == mosque_uuid

@pytest.mark.asyncio
async def test_options_flow_error_no_mosques_around(hass: HomeAssistant, config_entry_setup, mock_mosques_test_data):
...
@pytest.mark.asyncio
async def test_options_flow_no_input_reopens_form(hass: HomeAssistant, config_entry_setup, mock_mosques_test_data):
    ...
    with patch('homeassistant.components.mawaqit.config_flow.MawaqitPrayerOptionsFlowHandler.all_mosques_neighborhood', return_value={}), \
        ...
        assert result["type"] == data_entry_flow.FlowResultType.FORM  # Assert that a form is shown
        assert result["step_id"] == "init"

@pytest.mark.asyncio
async def test_options_flow_no_input_error_reopens_form(hass: HomeAssistant, config_entry_setup, mock_mosques_test_data):
...
async def test_write_all_mosques_NN_file(mock_file_io,test_folder_setup):
    ...
    with patch('homeassistant.components.mawaqit.config_flow.CURRENT_DIR', new='./test_dir'):
        mosques = [{"label": "Mosque A", "uuid": "uuid1"}]
        config_flow.write_all_mosques_NN_file(mosques)
        mock_file_io.assert_called_with(f"{test_folder_setup}/data/all_mosques_NN.txt", "w")
...
async def test_write_my_mosque_NN_file(mock_file_io,test_folder_setup):
    ...
    with patch('homeassistant.components.mawaqit.config_flow.CURRENT_DIR', new='./test_dir'):
        ...
        mock_file_io.assert_called_with(f"{test_folder_setup}/data/my_mosque_NN.txt", "w")
        assert mock_file_io().write.called, "The file's write method was not called."
